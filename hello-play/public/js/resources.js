...
$(document).ready(function() {
		...
		var table = $('<table id=\"data\"></table>').addClass('foo');
			...
			$.ajax({
                    type: "GET",
                    url: "http://localhost:9000/api/resources",
		...
		function print(response){
			for(var i=0;i<response._embedded.resources.length;i++)
			{
				var item = response._embedded.resources[i];
				var link = item._links.self.href;
				row = $('<tr ></tr>');
				$("#data").append(row);
				addToRow(row,item.description,link);
				addToRow(row,item.name,link);
